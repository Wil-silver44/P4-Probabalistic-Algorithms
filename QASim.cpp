...
void QASim::RunSim(int simNum, string testFile)
{
	...
	cout << setw(5) << right << "Base = IMPLEMENT MONTECARLO " << "Exponent = IMPLEMENT MONTECARLO" << endl;
	cout << setw(5) << right << "P(failure to detect bad item) = IMPLEMENT MONTECARLO" << endl;
        cout << setw(5) << right << "P(batch is good) = IMPLEMENT MONTECARLO" << endl;
	cout << setw(5) << right << "Percentage of bad batches detected = IMPLEMENT MONTECARLO" << endl << endl;	
}
